...
##Step 4a. Simulation function using mode imputation

missingness_performance <- function(simulations,missing_values) {
  ...
  for(i in 1:nsim){
    ...
    imputation_mode=sapply(train,Mode, na.rm = TRUE)
    for(m in c(4:22)){
      train[[m]][is.na(train[[m]])]=imputation_mode[m]
      validate[[m]][is.na(validate[[m]])]=imputation_mode[m]
    }
  ...
  #Merging all Performance Data into One Table and Adding Missing Values Column
  ...
  write.csv(summary_stats,file= paste("Output/Mode Imputation_Summary/Summary_Performance_", missing_values, ".csv", sep=""))
  write.csv(performance,file= paste("Output/Mode Imputation/Full_Performance_", missing_values, ".csv", sep=""))
}
